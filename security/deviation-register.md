...
| DEV-04 | Critical | infra | Molecule tests never execute in CI (B17) | `working-directory` bug in ansible-lint CI job prevents Molecule from finding role paths. | 9 Molecule scenarios exist but have never run. Broken roles (B16) went undetected. | Fix the `working-directory` in the ansible-lint CI workflow; land one green `molecule test` per role. | Principal |
| DEV-05 | Critical | infra | 5/10 Ansible roles declare unresolvable defaults (B16) | Jinja self-reference pattern `var: "{{ var \| default(...) }}"` in `defaults/main.yml` causes "Recursive loop detected" (rc 2). | postgresql, minio, vault, nginx, wireguard roles abort before contacting a host. | Rewrite 28 defaults as plain literals (app role already fixed as reference). | Principal |
