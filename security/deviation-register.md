...
| DEV-04 | ~~Critical~~ | infra | ~~Molecule tests never execute in CI (B17)~~ | ✅ Fixed: `working-directory` changed to `ansible/roles/${{ matrix.role }}`; monitoring/redis molecule.yml got missing `requirements-file`. | Was critical. | ✅ Fixed in PR #26 commit ded2e22. Awaiting first green CI run. | — |
